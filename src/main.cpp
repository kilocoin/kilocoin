...
const string strMessageMagic = "Kilocoin Signed Message:\n";
...
{
    // Kilocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 159 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 63072000); // Kilocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Kilocoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Kilocoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kilocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kilocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Kilocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 28/Jan/2014 - Pete Seeger, Songwriter and Champion of Folk Music, Dies at 94";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x379c60452be819c4b90b9d22ed185c2709e46db861ef12b43935f3ec3ba06109"));
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc0, 0xb6, 0xdb }; // Kilocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KilocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KilocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KilocoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running KilocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KilocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KilocoinMiner, pwallet));
}
